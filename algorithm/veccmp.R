...
source('vecutil.R')
## A lot of functions are directly copied from the compiler package
...
veccmpCall <- function(call, cntxt) {
    ...
    if("lapply" == as.character(fun) && isBaseVar("lapply", cntxt)) {
        ...
        f <- args[[2]]
        if(typeof(f) == "symbol") {
            #should find the function def, and change
            fDef <- findFunDef(f, cntxt)
            # TODO: generate the vectorizaiton of f
            vfDef <- vectranFun(fDef,cntxt) # do the transform 
            
            #define a new function symbol, and the name
            vf <- as.symbol(paste("v_", as.character(f), sep=''))
            
            e1 <- as.call(list(quote(`<-`), vf, vfDef))
            
            #wrapper list to vec
            v <- as.call(list(quote(list2vec), l))
            vcall <- as.call(list(vf, v))
            #now return a new structure vf(1)
            e2 <- as.call(list(quote(vec2list), vcall))
            #return as a whole
            as.call(list(quote(`{`), e1, e2))
        } else {
            
            # should define a new function here, then 
        }
...
# transformatio a function 
# cntxt: record the current vectorization's variable contxt
vectranFun <- function(fDef, cntxt) {
    body <- body(fDef)
...
veccompile <- function(e, env = .GlobalEnv, options = NULL) {
    cenv <- makeCenv(env)
